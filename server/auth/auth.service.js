...
function canMakeRequests() {
  return compose()
    .use(isAuthenticated())
    .use(function hasEnoughRequests(req,res,next){
      if (req.user.requests > 0){
        req.user.madeRequest();
        next();
      } else if (config.userRoles.indexOf(req.user.role) >= config.userRoles.indexOf('admin')){
        next();
      }
      else {
        res.status(406).send('Not enough requests');
      }
    });
}
...
function sendTokenToClient(req , res) {
  if (!req.user) return res.status(404).json({ message: 'Something went wrong, please try again.'});
  var token = signToken(req.user._id, req.user.role);
  let response = {};
